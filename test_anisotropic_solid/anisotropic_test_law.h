...
public:
  ...
  AnisotropicGeneralisedMooneyRivlin(double* nu_pt, double* c1_pt, double* c2_pt)
  ...
  AnisotropicGeneralisedMooneyRivlin(double* nu_pt, double* c1_pt, double* c2_pt, double* e_pt)
    : AnisotropicStrainEnergyFunction(),
      Nu_pt(nu_pt),
      C1_pt(c1_pt),
      C2_pt(c2_pt),
...
private:
  ...
  /// Dummy parameter for scaling of anisotropic influence
  double* C2_pt;
...
// Same as above but with an additional principal vector of anisotropy which represents the active stress
// and an additional "strain invariant" to communicate this active stress to the constitutive model
class ActiveAnisotropicGeneralisedMooneyRivlin : public AnisotropicStrainEnergyFunction
{
public:
  /// Constructor takes the pointers to the constitutive parameters:
  /// Poisson's ratio, the Mooney-Rivlin parameter. Young's modulus is set
  /// to 1, implying that it has been used to scale the stresses
  ActiveAnisotropicGeneralisedMooneyRivlin(double* nu_pt, double* c1_pt, double* c2_pt, double* c3_pt)
    : AnisotropicStrainEnergyFunction(),
      Nu_pt(nu_pt),
      C1_pt(c1_pt),
      C2_pt(c2_pt),
  ...
  /// Constructor takes the pointers to the constitutive parameters:
  /// Poisson's ratio, the Mooney-Rivlin parameter and Young's modulus
  ActiveAnisotropicGeneralisedMooneyRivlin(double* nu_pt, double* c1_pt, double* c2_pt, double* c3_pt, double* e_pt)
    : AnisotropicStrainEnergyFunction(),
      Nu_pt(nu_pt),
      C1_pt(c1_pt),
      C2_pt(c2_pt),
  ...
  /// Virtual destructor
  virtual ~ActiveAnisotropicGeneralisedMooneyRivlin()
  {
    if (Must_delete_e) delete E_pt;
  }
 ...
 {
  const unsigned dim = g.ncol();
  // The additional strain invariant for the principal vector of anisotropy: a_iG_{ij}a_j
  I[0] = 0.0;

  // The additional strain invariant which contains the active stress in the direction of the fibres
  I[1] = a[1][0];

  // Fill in the strain invariant and derivatives
  for(unsigned i=0; i<dim; i++)
  {
   for(unsigned j=0; j<dim; j++)
   {
  ...
  double W(const Vector<double>& I)
  {
    ...
    return 0.5 * ((*C1_pt) * (I[0] - 3.0) + (G - (*C1_pt)) * (I[1] - 3.0) +
                  ...
                  (1.0 - (*Nu_pt)) * G * (I[2] - 1.0) * (I[2] - 1.0) /
                    (2.0 * (1.0 - 2.0 * (*Nu_pt))))
           + (*C2_pt) * pow(I[3] - 1.0, 2.0) // the stress due to deformation in the fibre direction
           + (*C3_pt) * I[4] * I[3]; // the stress due to the active component
  }
  ...
  void derivatives(Vector<double>& I, Vector<double>& dWdI)
  {
    ...
    dWdI[2] = 0.5 * ((*C1_pt) - 2.0 * G +
                     2.0 * (1.0 - (*Nu_pt)) * G * (I[2] - 1.0) /
                       (2.0 * (1.0 - 2.0 * (*Nu_pt))));
    dWdI[3] = (*C2_pt) * 2.0 * (I[3] - 1.0) + (*C3_pt) * I[4];
    dWdI[4] = (*C3_pt) * I[3];
  }
...
private:
  ...
  /// Dummy parameter for scaling the active component
  double* C3_pt;

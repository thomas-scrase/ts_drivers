...
namespace Parameters
{
 ...
 // Define the principal vector of anisotropy. In the x-y plane and defined by some angle from the positive x direction.
 double Angle = 0.0;
...
CubicAnisotropicSolidProblem<ELEMENT>::CubicAnisotropicSolidProblem(const unsigned& nx, const unsigned& ny, const unsigned& nz)
{
 ...
 Parameters::anisotropic_strain_energy_function_pt = new AnisotropicGeneralisedMooneyRivlin(&Parameters::C1, &Parameters::C2, &Parameters::C3);
...
void CubicAnisotropicSolidProblem<ELEMENT>::doc_solution(const unsigned& label)
{
 ...
 sprintf(filename,"RESLT_CANTILEVER/soln%i.dat", label);
...
int main()
{
 ...
 // Vary gravity and output the solution
